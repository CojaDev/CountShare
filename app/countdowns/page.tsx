...
const CountdownsPage = () => {
    ...
    try {
      ...
      const countdownsWithCreatorNames = await Promise.all(
        fetchedCountdowns.map(async (countdown) => {
          const creatorResponse = await fetch(`/api/users/${countdown.createdBy}`)
  ...
  const filterAndSortCountdowns = () => {
    const filtered = countdowns.filter((countdown) => 
      countdown.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      countdown.description.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  const paginatedCountdowns = filteredCountdowns.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {paginatedCountdowns.map((countdown) => (
            <CountdownCard key={countdown._id} {...countdown} showActions={false} />
          ))}
        </div>
